...
class TestTreeRoots(unittest.TestCase):
    ...
    def test_call(self):
        ...
        for model_i in root_models:
            ...
            self.assertTrue(isclose(np.sum(root_pdf_1, axis=0) * self.dz, 1.0, rel_tol=1.0e-5))
            ...
            self.assertTrue(isclose(np.sum(root_pdf_1, axis=0) * self.dz,
                                    np.sum(root_pdf_2, axis=0) * self.dz,
                                    rel_tol=1.0e-4))
        # _end_for_
    ...
    def test_efficiency(self):
        ...
        # Simple version.
        theta_1d = np.random.rand(self.ln, 1)
        ...
        self.assertTrue(isclose(np.sum(rho_theta1, axis=0) * self.dz, 1.0, rel_tol=1.0e-5))

        # Vectorized version.
        theta_2d = np.random.rand(self.ln, 4)
        ...
        # Check if the integrated water efficiency sums to one.
        total_sum = np.sum(rho_theta2, axis=0) * self.dz
